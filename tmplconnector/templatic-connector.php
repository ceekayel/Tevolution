...
function templatic_legend_notification()
{
	...
	$legend_display .= '<p class="tevolution_desc">'.__('Email shortcodes are essentially variables you can use to display dynamic content. Shortcodes are specific to mails. Shortcodes in the mail content can be re-arranged or removed but new shortocdes can not be added from other Email Templates.',ADMINDOMAIN).'</p>';
...
function tevolution_author_renew_delete_link()
{
	...
	{			
		...
		{
			echo '<p class="package_expire"><strong>'.__('This listing is not published yet, contact site administrator for more details.',DOMAIN).'</strong></p>';
		}
		else
		{
...
function single_post_template_head()
{
	...
	<script  type="text/javascript" async >
		...
		if(function_exists('get_tevolution_login_permalink')){
			$store_login = "<a href='".get_tevolution_login_permalink()."'>".__('Sign in',DOMAIN)."</a>";
		}?>
		...
		var user_login_link ="<?php _e(' or ',DOMAIN); echo $store_login?>";
		var user_fname_verified="<?php _e(USER_AVAILABLE,DOMAIN);?>";
		var user_name_verified='';
		var user_name_error="<?php _e(INCORRECT_USER,DOMAIN); ?>";
		var submit_form_error="<?php _e("Please Login before you submit a form.",DOMAIN); ?>";
		
		var TWEET="<?php _e('Tweet',DOMAIN);?>";
		var FB_LIKE="<?php _e('Share',DOMAIN);?>";
...
function single_post_template_footer()
{
	...
	if((is_single() && in_array(get_post_type(),$custom_post_type)) ){
		?>
		<script  type="text/javascript" async src="<?php echo CUSTOM_FIELDS_URLPATH; ?>js/jquery.lightbox.js"></script>		
...
function include_currency_script()
{
    ...
    <script>
		...
		?>
		jQuery(window).load(function(){
			jQuery('#submit_form iframe').contents().find("html").css('direction','rtl');
		});
		<?php endif; ?>
	</script>
    <?php
...
if(!function_exists('tmpl_health_check')){
	function tmpl_health_check($dummy_data_msg = ''){
		...
		{
			echo '<div id="system-notification" class="tmpl-auto-install-yb" ><h4>'.__("System check for theme compatibility",ADMINDOMAIN).' </h4>
