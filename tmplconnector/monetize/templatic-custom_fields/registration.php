...
function retrieve_password() {
	...
	$message  .= '<p>'.sprintf(__('%s %s',DOMAIN),USERNAME_TEXT.": ", $user->user_login) . "</p>";
	$message .= '<p>'.sprintf(__('%s %s',DOMAIN), PASSWORD_TEXT.": " ,$new_pass) . "</p>";
	...
	$store_login_url = '<a href="'.$store_login.'">'.__('Login',DOMAIN).'</a>';
	$title = sprintf(__('[%s] Your new password',DOMAIN), get_option('blogname'));
 ...
 */
if(!function_exists('tevolution_register_new_user'))
{
	function tevolution_register_new_user($user_login, $user_email) {
		global $wpdb;
		$errors = new WP_Error();		
		...
		update_user_meta($user_id, 'first_name', $_POST['first_name']); // User Address Information Here
		update_user_meta($user_id, 'last_name', $_POST['last_name']); // User Address Information Here
		$userName = $_POST['first_name'].' '.$_POST['last_name'];
		
		$user_nicename = get_user_nice_name($_POST['first_name'],$_POST['last_name']); //generate nice name
		...
		if ( $user_id ) 
		{
			...
			update_user_meta($user_id, $wpdb->prefix.'capabilities', $user_role);
...
case 'retrievepassword' :
	...
	if ( isset($_GET['error']) && 'invalidkey' == $_GET['error'] ) $errors->add('invalidkey', __('Sorry, that key does not appear to be valid.',ADMINDOMAIN));
	do_action('lost_password');
	$message = '<div class="sucess_msg">'.__('Check your e-mail for your new password.',DOMAIN).'</div>';
...
case 'register' :
	...
	if ( $http_post ) {
		...
		$errors = tevolution_register_new_user($user_login, $user_email);
		if(is_wp_error($errors))
		{
		...
		if ( !is_wp_error($errors) ) 
		{
			...
			{
				$_REQUEST['reg_redirect_link']=get_author_posts_url( $errors[0]);
			}
...
<div class="content_spacer">
    <div class="form_col_1 fr">
        	<div class="form login_form">
	            <?php include(TEMPL_MONETIZE_FOLDER_PATH . "templatic-registration/login_form.php"); ?>
            </div>
        </div>
   		<div class="registration_col">
			   <?php include(TEMPL_MONETIZE_FOLDER_PATH . "templatic-registration/registration_form.php");?>
	    </div>
    </div>    
</div>
