	...
	<tbody>
        ...
        <th valign="top"><label for="days_for_no_post"><?php echo __('Allow users to submit listing within following days',ADMINDOMAIN);?><span class="required"><?php echo REQUIRED_TEXT; ?></span></label></th>
           <td>
    		    ...
    		    <p class="description"><?php echo __('Note: Submit listing within following days should be less than or equal to billing recurring package time.',ADMINDOMAIN);?></p>
           </td>
        </tr>
		...
		<tr id="rec_tr1" <?php if((isset($id) && get_post_meta($id, 'recurring', true) == 0)  || (!isset($id) || $id == '')){ echo 'style="display:none;"'; }?>>
			...
			<td>
				<input type="text" class="textfield" name="billing_cycle" id="billing_cycle" value="<?php if(isset($id) && $id != '') { echo get_post_meta($id, 'billing_cycle', true); } ?>"><br/><p class="description"><?php echo __('The number of times members will be billed, i.e. the number of times the process will be repeated. <strong>Note:</strong> With PayPal the number of payment cycles can range from 2 to 52. Enter 0 for unlimited cycles.',ADMINDOMAIN); ?></p>
			</td>
		</tr>
