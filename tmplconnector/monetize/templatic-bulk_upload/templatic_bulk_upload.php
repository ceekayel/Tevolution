...
if($_POST && isset($_SESSION['file_name']) && $_SESSION['file_name']!=""){
	...
	for($i=$start;$i<$loop; $i++){
		$session_count = $i;
		$postid = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_type='".$post_type."' and  post_title = '".mysql_real_escape_string($_SESSION['csv_data'][$i]['templatic_post_title'])."'" );		 
		if($postid==""):
			$new_post = array(
